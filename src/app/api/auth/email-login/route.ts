...
export async function POST(request: Request) {
  ...
  const backendRes = await fetch(`${env.apiBaseUrl}/auth/email/login`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  if (!backendRes.ok) {
    const errorBody = await backendRes.json().catch(() => ({}));
    return NextResponse.json({ message: errorBody?.message ?? "Sign-in failed." }, { status: backendRes.status });
  }
