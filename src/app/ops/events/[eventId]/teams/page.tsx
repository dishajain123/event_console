...
/** Same team hooks (`useEventTeams`, `useApproveTeam`, `useTeamMembers`, `useTeamJoinRequests`, `useTeamManagement`) and mutation payloads as before. */
              ...
              <TableBody>
                {teams.map((team) => (
                  <TableRow key={team.id}>
                    <TableCell className="font-medium text-[var(--foreground)]">{team.name}</TableCell>
                    <TableCell className="font-mono text-xs text-[var(--foreground-muted)]">{team.team_code}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex justify-end gap-2">
                        ...
                        {team.status === "submitted" && (
                          <Button size="sm" variant="outline" onClick={() => setConfirmTeam(team)}>Approve</Button>
                        )}
                      </div>
                    </TableCell>
                  </TableRow>
      ...
      {selectedTeam && (
        <GlassPanel className="mt-4">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-[var(--foreground)]">Manage {selectedTeam.name}</h2>
          ...
          <h3 className="mb-2 text-sm font-semibold text-[var(--foreground)]">Join requests</h3>
          {requestsLoading ? (
            <p className="text-sm text-[var(--foreground-muted)]">Loading requests…</p>
          ) : !requests?.length ? (
            <p className="mb-4 text-sm text-[var(--foreground-muted)]">No pending requests.</p>
          ) : (
            <div className="mb-4 space-y-2">
              {requests.filter((request) => request.status === "pending").map((request) => (
                <div key={request.id} className="flex items-center justify-between rounded-[var(--radius-sm)] border border-[var(--border)] p-3 text-sm">
                  <span className="text-[var(--foreground)]">{request.user_id}</span>
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      onClick={() =>
                        ask("Approve join request?", "The user will be added to this team.", async () => {
                          await management.respond.mutateAsync({ requestId: request.id, accept: true });
                        })
                      }
                    >
                      Approve
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        ask("Reject join request?", "The request will be rejected.", async () => {
                          await management.respond.mutateAsync({ requestId: request.id, accept: false });
                        })
                      }
                    >
                      Reject
          ...
          <h3 className="mb-2 text-sm font-semibold text-[var(--foreground)]">Members</h3>
          {membersLoading ? (
            <p className="text-sm text-[var(--foreground-muted)]">Loading members…</p>
          ) : !members?.length ? (
            <p className="text-sm text-[var(--foreground-muted)]">No members.</p>
          ) : (
            <div className="space-y-2">
              {members.map((member) => (
                <MemberRow
                  ...
                  member={member}
                  onRemove={() =>
                    ask("Remove member?", "This member will lose access to the team.", async () => {
                      await management.remove.mutateAsync(member.id);
                    })
                  }
                  onRoleChange={(role) => management.setRole.mutateAsync({ memberId: member.id, role })}
                  onAssignManager={() => management.assignManager.mutateAsync(member.user_id)}
                />
              ))}
            </div>
          )}
...
function MemberRow({
  ...
  onRemove: () => void;
  onRoleChange: (role: "manager" | "member") => Promise<unknown>;
  onAssignManager: () => Promise<unknown>;
}) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius-sm)] border border-[var(--border)] p-3 text-sm">
      <div>
        <span className="font-medium text-[var(--foreground)]">{member.full_name}</span>
        <span className="ml-2 text-xs text-[var(--foreground-muted)]">{member.role}</span>
      </div>
      {!member.is_captain && (
        <div className="flex gap-2">
          <Button size="sm" variant="ghost" onClick={() => void onRoleChange(member.role === "manager" ? "member" : "manager")}>
            {member.role === "manager" ? "Remove manager" : "Make manager"}
          </Button>
          <Button size="sm" variant="outline" onClick={() => void onAssignManager()}>
            Assign manager
          </Button>
